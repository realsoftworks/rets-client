...
import { IClientObjects } from "./object";
...
export interface RequestDebugFunction {
  (type: 'request' | 'response' | 'redirect' | 'auth', data: any): void;
}

export interface IClientSettingsWithUAAuthorization
  extends Omit<IClientConfiguration, 'username' | 'password'> {
...
export type ClientSettings =
  ...
  | IClientSettingsWithUAAuthorization;
...
export interface IGetObjectResponse extends IBaseResponse {
  results: {
    info?: any;
...
export interface IRetsStreamDoneEvent
  extends IRetsStreamEvent<{
    rowsReceived: number;
  }> {
  type: 'done';
}
export interface IRetsStreamErrorEvent
  extends IRetsStreamEvent<RetsReplyError> {
  type: 'error';
}
...
export interface IRetsStreamHeaderInfoEvent
  extends IRetsStreamEvent<IHeaderInfo> {
  type: 'headerInfo';
}
